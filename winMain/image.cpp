...
void image::alphaInit()
{
	...
	_blendImage->hBit = (HBITMAP) CreateCompatibleBitmap( 
		hdc, this->_imageInfo->width, this->_imageInfo->height );
...
void image::render( HDC hdc )
{
	if ( _trans )
	{
		GdiTransparentBlt( hdc,		//복사할 장소의 DC
			_imageInfo->x,			//복사될 좌표 시작지점 X
			_imageInfo->y,			//복사될 좌표 시작지점 Y
	...
	else
	{
		...
		BitBlt( hdc,				//1. 그릴 도화지
			_imageInfo->x,			//2. 그릴 도화지 x
			_imageInfo->y,			//3. 그릴 도화지 y
...
void image::render( HDC hdc, int destX, int destY, int sourX, int sourY, int orgWidth, int orgHeight, int sourWidth, int sourHeight )
{
	if ( _trans )
	{
		GdiTransparentBlt( hdc, destX, destY, sourWidth, sourHeight,
			_imageInfo->hMemDC, sourX, sourY, orgWidth, orgHeight, _transColor );
	}
	else
	{
		BitBlt( hdc, destX, destY, sourWidth, sourHeight,
